...
Lângă formular, panoul **Vreme locală** afișează temperatura, umiditatea și condițiile curente la poziția stației (latitudine/longitudine din profilul operatorului), preluate de la [Open-Meteo](https://open-meteo.com/) — public, fără cheie API. Presiunea atmosferică, viteza și direcția vântului provin din observația METAR a Aeroportului Internațional Sibiu (LRSB), preluată de la Aviation Weather Center; presiunea este afișată în hPa, viteza atât în noduri, cât și în km/h, iar direcția în grade și punct cardinal — sau `Variabilă`, dacă METAR raportează vânt cu direcție variabilă (`VRB`, tipic la vânt slab) în loc de un cap busolă, deosebit explicit de o valoare indisponibilă. O observație METAR obișnuită se reemite doar la 30–60 de minute, așa că observația e păstrată local în `cache/local_weather/sibiu_metar.json` timp de 20 de minute — o reîmprospătare (automată sau manuală) în această fereastră reutilizează cache-ul în loc să solicite din nou serverul Aviation Weather Center la fiecare ciclu, ceea ce ar risca un blocaj/limitare din partea acestuia la intervale scurte (1–5 minute). Dacă o cerere nouă eșuează, panoul revine la ultima observație validă din cache (până la 3 ore vechime) în loc să afișeze N/A pentru o eroare trecătoare; presiunea și vântul apar ca `N/A` doar dacă nici cache-ul nu (mai) are o valoare utilizabilă, fără a ascunde celelalte date meteo. Panoul se actualizează automat, o singură dată la scurt timp după deschiderea aplicației, apoi periodic la fiecare `local_weather_auto_refresh_minutes` (vezi [Setări vreme locală](#setări-vreme-locală)) minute; butonul **Actualizează** apare doar dacă actualizarea automată este dezactivată din setări. Dacă poziția stației nu e setată (Setări → Date operator), panoul arată acest lucru în loc să încerce o cerere fără sens. La fel ca panoul de propagare, orice eșec de rețea lasă ultimele valori afișate neschimbate, cu un mesaj de stare clar.
...
Activitatea aurorală, Bt și densitatea/temperatura vântului solar nu sunt urmărite: nu intră în calculul estimărilor de bandă din tabel, iar pentru ele nu există o sursă publică la fel de simplă și verificabilă ca restul furnizorilor de mai sus.
