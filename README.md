...
Aplicaţia descarcă exclusiv JSON public HTTPS de la NOAA SWPC: `planetary_k_index_1m.json`, `f107_cm_flux.json`, `predicted_f107_cm_flux.json` şi `alerts.json`. Sunt folosite SFI/F10.7, Kp, indicele A din câmpul `a_running` al răspunsului planetary K-index, numărul petelor (dacă este prezent) şi alertele de blackout. Endpointul retras `wing_kp_1m.json` nu mai este solicitat. NOAA primeşte numai cereri pentru date globale: nu sunt trimise indicativul, numele, adresa sau coordonatele. Harta PNG se generează local din poziţia deja configurată a operatorului; coordonatele exacte nu sunt tipărite şi nu sunt puse în loguri.
