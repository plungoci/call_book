...
def maidenhead_to_coordinates(locator: str) -> tuple[float, float]:
    ...
    if not ("A" <= text[0] <= "R" and "A" <= text[1] <= "R"):
        raise ValueError("Locator Maidenhead invalid.")
    lon = (ord(text[0]) - 65) * 20.0 - 180
    lat = (ord(text[1]) - 65) * 10.0 - 90
    lon += int(text[2]) * 2
    lat += int(text[3])
    lon_size, lat_size = 2.0, 1.0
    if len(text) >= 6:
        if not ("A" <= text[4] <= "X" and "A" <= text[5] <= "X"):
            raise ValueError("Locator Maidenhead invalid.")
        lon_size, lat_size = 2 / 24, 1 / 24
        lon += (ord(text[4]) - 65) * lon_size
        lat += (ord(text[5]) - 65) * lat_size
    if len(text) == 8:
        if not text[6:].isdigit():
            raise ValueError("Locator Maidenhead invalid.")
        lon_size, lat_size = (2 / 24) / 10, (1 / 24) / 10
        lon += int(text[6]) * lon_size
        lat += int(text[7]) * lat_size
