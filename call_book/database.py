...
class Database:
    def __init__(self, path: Path = Path("data/logbook.db")):
        self.path = path
...
CREATE TABLE IF NOT EXISTS repeaters (
    id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, output_frequency_mhz REAL NOT NULL,
    input_frequency_mhz REAL, shift_mhz REAL, tone_hz REAL, mode TEXT, location TEXT,
    grid_square TEXT, notes TEXT
);
CREATE TABLE IF NOT EXISTS qsos (
    id INTEGER PRIMARY KEY AUTOINCREMENT, callsign TEXT NOT NULL, qso_start_utc TEXT NOT NULL,
    qso_end_utc TEXT, frequency_mhz REAL NOT NULL, band TEXT, mode TEXT NOT NULL, repeater_id INTEGER,
    rst_sent TEXT, rst_received TEXT, operator_name TEXT, grid_square TEXT, my_grid_square TEXT,
    power_w REAL, notes TEXT, qsl_status TEXT DEFAULT 'NOT_SENT', created_at TEXT NOT NULL, updated_at TEXT,
    FOREIGN KEY(repeater_id) REFERENCES repeaters(id) ON DELETE SET NULL
);
CREATE TABLE IF NOT EXISTS stations (
    id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, callsign TEXT NOT NULL,
    location TEXT, grid_square TEXT, equipment TEXT, antenna TEXT
);""")
        with self.connect() as c:
            c.execute("""
CREATE TABLE IF NOT EXISTS operator_profile (
    id INTEGER PRIMARY KEY CHECK (id = 1), callsign TEXT NOT NULL DEFAULT '', full_name TEXT NOT NULL DEFAULT '',
    maidenhead_locator TEXT NOT NULL DEFAULT '', locality TEXT NOT NULL DEFAULT '', county TEXT NOT NULL DEFAULT '',
    country TEXT NOT NULL DEFAULT '', address TEXT NOT NULL DEFAULT '', email TEXT NOT NULL DEFAULT '',
    phone TEXT NOT NULL DEFAULT '', radio_equipment TEXT NOT NULL DEFAULT '', antenna TEXT NOT NULL DEFAULT '',
    default_power_w REAL, radio_club TEXT NOT NULL DEFAULT '', club_callsign TEXT NOT NULL DEFAULT '',
    notes TEXT NOT NULL DEFAULT ''
            ...
            self._add_missing_columns(
                ...
                "operator_profile",
                {
                    ...
                    "maidenhead_locator": "TEXT NOT NULL DEFAULT ''",
                    ...
                    "radio_equipment": "TEXT NOT NULL DEFAULT ''",
                    "antenna": "TEXT NOT NULL DEFAULT ''",
                    "default_power_w": "REAL",
                    "radio_club": "TEXT NOT NULL DEFAULT ''",
                    "club_callsign": "TEXT NOT NULL DEFAULT ''",
                    "notes": "TEXT NOT NULL DEFAULT ''",
                    "latitude": "REAL",
                    "longitude": "REAL",
            ...
            self._add_missing_columns(
                ...
                {
                    "my_grid_square": "TEXT",
                    "propagation_mode": "TEXT NOT NULL DEFAULT 'Necunoscută'",
                    "satellite_name": "TEXT",
                    "uplink_mode": "TEXT",
                    "downlink_mode": "TEXT",
                    "distance_km": "REAL",
    ...
    def get_operator_profile(self) -> OperatorProfile:
        ...
        return (
            OperatorProfile(**{key: row[key] for key in OperatorProfile.__dataclass_fields__})
            if row
    ...
    def save_operator_profile(self, profile: OperatorProfile) -> None:
        ...
        fields = list(OperatorProfile.__dataclass_fields__)
        values = [getattr(profile, field) for field in fields]
        assignments = ", ".join(f"{field}=excluded.{field}" for field in fields)
        with self.connect() as c:
            c.execute(
                f"INSERT INTO operator_profile (id,{','.join(fields)}) "
    ...
    def save_qso(self, q: QSO) -> int:
        ...
        with self.connect() as c:
            if q.id:
                c.execute(
                    f"UPDATE qsos SET {','.join(f'{x}=?' for x in fields.split(','))},updated_at=? WHERE id=?",
                    vals + [now, q.id],
                )
                return q.id
    ...
    def list_qsos(self, filters: dict[str, str] | None = None) -> list[sqlite3.Row]:
        ...
        for key, col in (
            ...
            ("band", "q.band"),
            ("mode", "q.mode"),
            ("repeater_id", "q.repeater_id"),
        ):
            if filters.get(key):
                where.append(f"{col} LIKE ?" if key == "callsign" else f"{col}=?")
                values.append(f"%{filters[key].upper()}%" if key == "callsign" else filters[key])
        for key, op in (("date_from", ">="), ("date_to", "<=")):
            if filters.get(key):
                where.append(f"q.qso_start_utc {op} ?")
                values.append(filters[key] + ("T23:59:59+00:00" if key == "date_to" else "T00:00:00+00:00"))
        sql = (
    ...
    def delete_qso(self, id: int):
        with self.connect() as c:
            c.execute("DELETE FROM qsos WHERE id=?", (id,))

    def _one(self, sql, values):
        with self.connect() as c:
            row = c.execute(sql, values).fetchone()
            if not row:
                raise KeyError("Înregistrare inexistentă")
            return row
    ...
    def save_repeater(self, r: Repeater) -> int:
        ...
        vals = [getattr(r, x) for x in names.split(",")]
        with self.connect() as c:
            if r.id:
                c.execute(
                    f"UPDATE repeaters SET {','.join(x + '=?' for x in names.split(','))} WHERE id=?", vals + [r.id]
                )
                return r.id
    ...
    def delete_repeater(self, id: int):
        ...
        with self.connect() as c:
            c.execute("DELETE FROM repeaters WHERE id=?", (id,))
