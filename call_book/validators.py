...
def validate_callsign(value: str) -> str:
    ...
    if not value or not CALLSIGN_RE.fullmatch(value) or not any(c.isalpha() for c in value):
        raise ValueError("Indicativul este obligatoriu și poate conține litere, cifre și '/'.")
    return value
...
def parse_positive(value: str, label: str) -> float:
    try:
        ...
        raise ValueError(f"{label} trebuie să fie numerică.") from exc
    if number <= 0:
        raise ValueError(f"{label} trebuie să fie mai mare decât zero.")
    return number
