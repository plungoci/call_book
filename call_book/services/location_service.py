...
class LocationNetworkError(LocationError):
    pass


class LocationDnsError(LocationNetworkError):
    pass


class LocationInternetError(LocationNetworkError):
    pass


class LocationTlsError(LocationNetworkError):
    pass


class LocationHttpError(LocationNetworkError):
    pass


class LocationResponseError(LocationNetworkError):
    pass
...
def _coordinates(data: dict) -> tuple[float, float, float | None]:
    ...
    try:
        # A missing coordinate is `None` here by design: float(None) raises
        # TypeError, which the except clause below turns into a clear error.
        latitude, longitude = float(latitude), float(longitude)  # type: ignore[arg-type]
...
class IpLocationService:
    ...
    def locate(self, timeout_seconds: int | None = None) -> LocationResult:
        ...
        try:
            ...
            with socket.create_connection(addresses[0][4], timeout=timeout):  # type: ignore[arg-type]
                pass
        ...
        return LocationResult(latitude, longitude, accuracy, "Geolocalizare IP (ipwho.is)", datetime.now(UTC))
...
@{
    latitude=$p.Coordinate.Point.Position.Latitude
    longitude=$p.Coordinate.Point.Position.Longitude
    accuracy=$p.Coordinate.Accuracy
    ...
    def locate(self, timeout_seconds: int | None = None) -> LocationResult:
        ...
        return LocationResult(latitude, longitude, accuracy, "Windows Location", datetime.now(UTC))
