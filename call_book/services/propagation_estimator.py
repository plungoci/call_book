...
def evaluate_band_conditions(
    ...
    available = sum(
        value is not None for value in (space_weather.solar_flux, space_weather.kp_index, space_weather.a_index)
    )
    ...
    return BandCondition(
        rating, score, f"Estimare simplificată: {rating.lower()} ({score:.0f}/100).", tuple(warnings), confidence
    )
