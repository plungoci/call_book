...
def adif_record(q:QSO, profile: OperatorProfile | None = None)->str:
 ...
 if q.propagation_mode in ADIF_PROPAGATION_MODES: values["PROP_MODE"] = ADIF_PROPAGATION_MODES[q.propagation_mode]
 if q.satellite_name: values["SAT_NAME"] = q.satellite_name
 if q.uplink_mode or q.downlink_mode: values["SAT_MODE"] = "/".join(x for x in (q.uplink_mode, q.downlink_mode) if x)
 if q.distance_km is not None: values["DISTANCE"] = f"{q.distance_km:g}"
 # ADIF has only one general comment field; preserve existing QSO notes first.
 if q.propagation_notes: values["COMMENT"] = "\n".join(x for x in (q.notes, q.propagation_notes) if x)
