...
def validate_qso(qso: QSO) -> QSO:
    ...
    if qso.propagation_mode not in PROPAGATION_MODES: raise ValueError("Tip de propagare invalid.")
    if qso.distance_km is not None and qso.distance_km <= 0: raise ValueError("Distanța trebuie să fie mai mare decât zero.")
    if qso.azimuth_deg is not None and not 0 <= qso.azimuth_deg <= 360: raise ValueError("Azimutul trebuie să fie între 0 și 360 de grade.")
    if qso.propagation_mode == SATELLITE_PROPAGATION:
        if not qso.satellite_name.strip(): raise ValueError("Numele satelitului este obligatoriu pentru propagarea prin satelit.")
        if not qso.uplink_mode.strip() or not qso.downlink_mode.strip(): raise ValueError("Modurile uplink și downlink sunt obligatorii pentru propagarea prin satelit.")
