...
class PropagationPanel(QGroupBox):
    def __init__(self,parent=None):
        ...
        self.table=QTableWidget(5,6);self.table.setHorizontalHeaderLabels(('Bandă','Interval frecvență','Zi','Noapte','Scor','Încredere'));self.table.setVerticalHeaderLabels(('80m','40m','20m','15m','10m'));self.table.verticalHeader().setVisible(False);l.addWidget(self.table)
    def schedule(self,band,frequency=None,delay=700):
        if band.strip():self.timer.start(delay)
    def refresh(self,force=True):
        self.button.setEnabled(False);self.status.setText('Se descarcă date…');thread=QThread(self);self._worker_thread=thread;self.worker=Worker(force);self.worker.moveToThread(thread);thread.started.connect(self.worker.run);self.worker.finished.connect(self.update_values);self.worker.finished.connect(thread.quit);self.worker.failed.connect(lambda:self.status.setText('Ultima actualizare nu a reușit.'));self.worker.failed.connect(thread.quit);thread.finished.connect(lambda:self.button.setEnabled(True));thread.start()
    def update_values(self,w):
        ...
        for r,(band,(day,night)) in enumerate(self.estimator.calculate_hf(w,datetime.now(timezone.utc)).items()):
            values=(band,frequency_range_for_band(band) or 'N/A',day.rating,night.rating,f'{(day.score+night.score)/2:.0f}/100',day.confidence.capitalize())
            for c,v in enumerate(values):self.table.setItem(r,c,QTableWidgetItem(v))
    def shutdown(self):
        self.timer.stop()
        if self._worker_thread is not None and self._worker_thread.isRunning():self._worker_thread.quit();self._worker_thread.wait(1000)
