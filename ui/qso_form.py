...
class QSOForm(ttk.LabelFrame):
 ...
 def _build(self):
  ...
  for i,(label,key) in enumerate(labels):
   ...
   widget.grid(row=i//2*2+1,column=i%2*2,sticky="ew",padx=3);setattr(self,key+"_widget",widget)
  ttk.Label(self,text="Observații").grid(row=14,column=0,sticky="w");self.notes=tk.Text(self,width=65,height=3);self.notes.grid(row=15,column=0,columnspan=4,sticky="ew",padx=3)
